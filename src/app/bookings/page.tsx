...
export default function BookingsPage() {
  ...
  const cardBase = cn('rounded-2xl border p-4 flex items-center gap-3', d ? 'bg-[#001117] border-[#E6A854]/20' : 'bg-white border-[#E5E7EB]')

  return (
    <div className={cn('flex-1 min-h-screen', d ? 'bg-[#001117]' : 'bg-[#F9FAFB]')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="mb-6 sm:mb-8">
          <h1 className={cn('text-xl sm:text-2xl font-bold', d ? 'text-white' : 'text-slate-800')}>Booking Requests</h1>
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
          {[
            ...
            <div key={label} className={cardBase}>
              ...
              <div>
                <p className={cn('text-xl sm:text-2xl font-bold', d ? 'text-white' : 'text-slate-800')}>{value}</p>
    ...
    <div className={cn('rounded-2xl border p-4 sm:p-5', d ? 'bg-[#001117] border-[#E6A854]/20' : 'bg-white border-[#E5E7EB]')}>
      ...
      {booking.status === 'pending' && onApprove && onReject && (
        <div className={cn('flex gap-2 pt-3 border-t', d ? 'border-[#E6A854]/20' : 'border-[#E5E7EB]')}>
          <button onClick={onReject} className={cn('flex-1 py-2 border text-sm font-medium rounded-xl transition', d ? 'border-[#E6A854]/30 text-slate-400 hover:bg-red-500/10 hover:text-red-400' : 'border-[#E5E7EB] text-slate-500 hover:bg-red-50 hover:text-red-500 hover:border-red-200')}>Reject</button>
