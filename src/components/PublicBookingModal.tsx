...
export default function PublicBookingModal({ property, mode = 'rent', onClose }: Props) {
  ...
  const inputCls = cn(
    ...
    d ? 'bg-[#001117]/50 border-[#E6A854]/30 text-white placeholder-slate-500 focus:ring-[#E6A854]/50 focus:border-[#E6A854]'
      : 'bg-white border-[#E5E7EB] text-slate-700 placeholder-slate-300 focus:ring-slate-200 focus:border-slate-400'
  )
        ...
        <div className="p-6">
          ...
          {mode === 'rent' ? (
            <div className="grid grid-cols-2 gap-3 mb-4">
              {[{ label: 'Harga/Bulan', value: formatCurrency(property.price_monthly) }, { label: 'Harga/Hari', value: formatCurrency(property.price_daily) }].map(({ label, value }) => (
          ...
          {success ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className={cn('text-xs font-medium mb-1 flex items-center gap-1', subtext)}><User className="w-3 h-3" /> Nama Lengkap</label>
                <input type="text" value={form.customerName} onChange={(e) => setForm({ ...form, customerName: e.target.value })} placeholder="Contoh: Budi Santoso" className={inputCls} />
                {errors.customerName && <p className="text-xs text-red-500 mt-1">{errors.customerName}</p>}
              </div>

              <div>
                <label className={cn('text-xs font-medium mb-1 flex items-center gap-1', subtext)}><Mail className="w-3 h-3" /> Email</label>
                <input type="email" value={form.customerEmail} onChange={(e) => setForm({ ...form, customerEmail: e.target.value })} placeholder="contoh@email.com" disabled={isAuthenticated} className={cn(inputCls, 'disabled:opacity-50')} />
                {errors.customerEmail && <p className="text-xs text-red-500 mt-1">{errors.customerEmail}</p>}
              </div>

              <div>
                <label className={cn('text-xs font-medium mb-1 flex items-center gap-1', subtext)}><Phone className="w-3 h-3" /> Nomor Telepon (Opsional)</label>
                <input type="tel" value={form.customerPhone} onChange={(e) => setForm({ ...form, customerPhone: e.target.value })} placeholder="08123456789" className={inputCls} />
              </div>

              {mode === 'rent' && (
                <>
                  <div>
                    <label className={cn('text-xs font-medium mb-1 flex items-center gap-1', subtext)}><Calendar className="w-3 h-3" /> Tanggal Check-in</label>
                    <div className="relative">
                      <input type="date" value={form.checkIn} onChange={(e) => setForm({ ...form, checkIn: e.target.value })} className={cn(inputCls, 'pr-10')} />
                      {form.checkIn && (
                  ...
                  <div>
                    <label className={cn('text-xs font-medium mb-1 flex items-center gap-1', subtext)}><Clock className="w-3 h-3" /> Durasi Sewa (bulan)</label>
                    <input type="number" min="1" value={form.duration} onChange={(e) => setForm({ ...form, duration: e.target.value })} className={inputCls} />
